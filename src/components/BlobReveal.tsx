...
const CAMERA_FOV = 35;
/* O vertex shader não desloca vértices: todo o efeito é calculado no
   fragment shader, então o plano não precisa de subdivisões (128×128
   segmentos ≈ 33 mil vértices recriados a cada resize, sem ganho visual). */
...
export default function BlobReveal({
    ...
    const initThree = useCallback(() => {
        ...
        /* antialias desativado: o MSAA só suaviza arestas de geometria, e as
           bordas dos blobs são resolvidas dentro do fragment shader — o custo
           extra não altera a qualidade do efeito. */
    ...
    /* A animação dispara apenas pelo efeito de scroll abaixo (quando a seção
       entra na tela), evitando rodar o tween no carregamento da página com o
       componente fora do viewport. */
    ...
    useEffect(() => {
        ...
        if (inView && progress < 0.99) {
            ...
            /* Sem loop de rAF paralelo: o onUpdate do gsap já renderiza um
               frame por tick, então o loop dobrava as renderizações. */
